...
以下专题从用户给定的四个 Expo 官方入口分别起步，独立编号、独立沿官方页脚 Next 继续。`versions-latest` 从滚动更新的 SDK Latest 参考页开始，随后沿 Next 延伸到第三方库、技术规范和 Expo CLI 工具页；有版本化参考页时对照本项目的 SDK 56，未版本化页面则标明其版本范围。访问时 Latest 稳定版本为 SDK 57.0.0，本项目 Expo 为 `~56.0.11`。
